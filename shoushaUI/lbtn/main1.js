app.import(function (lib, game, ui, get, ai, _status, app) {
  ...
  lib.arenaReady.push(function () {
    ...
    head.style.cssText = "display: block;--w: 50px;--h: calc(var(--w) * 81/247);width: var(--w);height: var(--h)+4px;position: absolute;top: calc(100% - 50px);left: calc(100% - 370px);background-color: transparent;z-index:4"
    ...
    head.style.cssText = "display: block;--w: 56px;--h: calc(var(--w) * 74/71);width: var(--w);height: var(--h);position: absolute;bottom: calc(100% - 69px);left: calc(100% - 120px);background-color: transparent;z-index:1"
    head.onclick = function () {
    ...
    head.onclick = function () {
      ...
      BJ.addEventListener('click', event => {
        ...
        //换背景          
        var Backgrounds = [ "华灯初上", "天书乱斗", "游卡桌游", "十周年"];
  ...
  var plugin = {
    ...
    create: {
      ...
      cardRoundTime: function () {
        var node = ui.create.div('.cardRoundNumber', ui.arena, plugin.click.huanfu).hide();
        node.node = {
