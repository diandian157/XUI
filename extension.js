...
import { versus } from "./extension/versus.js";
import { cardkmh } from "./extension/cardkmh.js";
...
export default async function () {
	...
	return {
		...
		content(config, pack) {
			...
			//-----------目标选中---------//
			if(config.shoupaikeshi){
				lib.skill._shoupaikeshi={
					mode: ["doudizhu",'versus','boss'],
					locked: true,
					ai: {
					    viewHandcard: true,
						...
						element: {
							...
							player: {
								...
								$dieAfter: function () {
									...
									var url = decadeUIPath + "image/decoration" + (goon ? "/dead" : "s/dead") + "_" + identity + ".png";
									image.onerror = function () {
			...
			*/
				...
				if (lib.config["extension_十周年UI_enable"] && lib.config.extension_十周年UI_jindutiao == true) {
					if (!document.querySelector("#jindutiaopl") && that == game.me) {
						...
						if (!tipbanlist.includes(name) && lib.config.extension_十周年UI_newDecadeStyle != 'on') {
							...
							tipshow.style.cssText = "display:block;position:absolute;z-index:91;--w: 133px;--h: calc(var(--w) * 50/431);left:27px;width: var(--w);height: var(--h);bottom:-22px;";
							...
							content: function () {
								...
								if (a.length <= 0) {
									...
									if (lib.config.extension_十周年UI_newDecadeStyle != 'on') {
										...
										tipAB.style.cssText = "display:block;position:absolute;z-index:91;--w: 133px;--h: calc(var(--w) * 50/431);left:27px;width: var(--w);height: var(--h);bottom:-22px;";
									}
									else {
										tipAB.src = lib.assetURL + 'extension/十周年UI/shoushaUI/lbtn/images/shoushatip/phasetip.png';
										tipAB.classList.add("playertip")
										tipAB.style.cssText = "display:block;position:absolute;z-index:92;--w: 129px;--h: calc(var(--w) * 50/431);left:25px;width: var(--w);height: var(--h);bottom:-9.2px;transform:scale(1.2);";
									}
							...
							content: function () {
								...
								if (a.length <= 0) {
									...
									if (lib.config.extension_十周年UI_newDecadeStyle != 'on') {
										...
										tipCD.style.cssText = "display:block;position:absolute;z-index:91;--w: 133px;--h: calc(var(--w) * 50/431);left:27px;width: var(--w);height: var(--h);bottom:-22px;";
									}
									else {
										tipCD.src = lib.assetURL + 'extension/十周年UI/shoushaUI/lbtn/images/shoushatip/discardtip.png';
										tipCD.classList.add("playertipQP")
										tipCD.style.cssText = "display:block;position:absolute;z-index:92;--w: 129px;--h: calc(var(--w) * 50/431);left:25px;width: var(--w);height: var(--h);bottom:-9.2px;transform:scale(1.2);";
			...
			lib.skill._chupaiE = {
							...
							content: function () {
								...
								if (d.length <= 0) {
									...
									tipEF.style.cssText = "display:block;position:absolute;z-index:91;--w: 133px;--h: calc(var(--w) * 50/431);left:27px;width: var(--w);height: var(--h);bottom:-22px;";
			...
			lib.skill._chupaiG = {
							...
							content: function () {
								...
								if (e.length <= 0) {
									...
									tipGH.style.cssText = "display:block;position:absolute;z-index:91;--w: 133px;--h: calc(var(--w) * 50/431);left:27px;width: var(--w);height: var(--h);bottom:-22px;";
			...
			lib.skill._chupaiM = {
							...
							content: function () {
								...
								if (k.length <= 0) {
									...
									tipMN.style.cssText = "display:block;position:absolute;z-index:91;--w: 133px;--h: calc(var(--w) * 50/431);left:27px;width: var(--w);height: var(--h);bottom:-22px;";
			...
			lib.skill._chupaiO = {
							...
							content: function () {
								...
								if (n.length <= 0) {
									...
									tipOP.style.cssText = "display:block;position:absolute;z-index:91;--w: 133px;--h: calc(var(--w) * 50/431);left:27px;width: var(--w);height: var(--h);bottom:-22px;";
			...
			lib.skill._chupaiI = {
							...
							content: function () {
								...
								if (g.length <= 0) {
									...
									tipIJ.style.cssText = "display:block;position:absolute;z-index:91;--w: 133px;--h: calc(var(--w) * 50/431);left:27px;width: var(--w);height: var(--h);bottom:-22px;";
			...
			lib.skill._chupaiMX = {
				...
				content: function () {
					...
					if (d.length <= 0) {
						...
						tipMNX.style.cssText = "display:block;position:absolute;z-index:92;--w: 129px;--h: calc(var(--w) * 50/431);left:25px;width: var(--w);height: var(--h);bottom:-9.2px;transform:scale(1.2);";
			...
			if (config.GTBB) {
				...
				var gddf = function () {
					...
					//更改对应播报颜色
					var gold = ['<font color="#56e4fa">' + pifu + "</font>", '<font color="#f3c20f">' + wujiang + "</font>"].randomGet();
		...
		precontent() {
					...
					playerSkills: function (node, arg1, arg2) {
						...
						skills.addArray(
							Object.keys(node.disabledSkills).filter(function (k) {
								return !node.hiddenSkills.includes(k) && node.disabledSkills[k].length && node.disabledSkills[k][0] === k + "_awake";
							})
						);
						return skills;
					},
			...
			game.Jindutiaoplayer = function () {
				...
				if (lib.config.extension_十周年UI_jindutiaoYangshi == "3") {
					...
					boxTime.style.cssText = "z-index:1;width:614px;height:8px;margin:0 0 0 1px;background-color: #F4C336;border-top:3px solid #EBE1A7;border-bottom:2px solid #73640D;border-left:1px solid #73640D;position: absolute;top: 0px;border-radius:3px;";
					boxContent.appendChild(boxTime);
		...
		config: {
			...
			"shoupaikeshi":{
				name:'手牌可视',
				init:true,
				intro:'开启后，22和斗地主模式队友的手牌始终对玩家可见（右击或双击武将牌查看），重启生效。',
			},
			//手杀UI
			FL1: {
				"name": "<img style=width:240px src=" + lib.assetURL + "extension/十周年UI/shoushaUI/line.png>",
